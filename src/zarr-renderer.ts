...
export class ZarrRenderer {
  ...
  constructor(gl: WebGL2RenderingContext, fragmentShaderSource: string) {
    ...
    this.useFillValueLoc = mustGetUniformLocation(this.gl, program, "u_useFillValue");
    this.fillValueLoc = mustGetUniformLocation(this.gl, program, "u_fillValue");
    this.scaleFactorLoc = mustGetUniformLocation(this.gl, program, "u_scaleFactor");
    ...
    const renderProgram = createProgram(this.gl, renderVertShader, renderFragShader);
    if (!renderProgram) {
      ...
      for (const tileTuple of visibleTiles) {
        ...
        if (tile && tile.data) {
          ...
          if (parent) {
            ...
            texCoords = this.getOverzoomTexCoords(
              z,
              x,
              y,
              parent.ancestorZ
